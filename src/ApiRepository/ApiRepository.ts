...
export const getPendingFine = (): Promise<any> => {
  return axios.get(`${baseApi}/multa/pendientes/${sessionStorage.getItem('user') ? JSON.parse(sessionStorage.getItem('user') as string).rutUsuario : ''}`);
};
...
export const putPaymentExpense = (idGasto: number, status: string): Promise<any> => {
  return axios.put(`${baseApi}/gasto-comun/actualizar-estado`, {
    idGasto: idGasto,
    nuevoEstado: status,
    rutUsuario: sessionStorage.getItem('user') ? JSON.parse(sessionStorage.getItem('user') as string).rutUsuario : ''
  });
};

export const putPaymentFine = (idMulta: number, status: string): Promise<any> => {
  return axios.put(`${baseApi}/multa/actualizar-estado`, {
    idMulta: idMulta,
    nuevoEstado: status,
    rutUsuario: sessionStorage.getItem('user') ? JSON.parse(sessionStorage.getItem('user') as string).rutUsuario : ''
  });
